...
import math
from .models import *
from air_config.utils import render_to_pdf, createticket


#To'lov va qo'shimcha haq uchun 
from .constant import FEE
from sky_app.utils import createWeekDays, addPlaces, addDomesticFlights, addInternationalFlights
...
try:
    ...
    if len(Flight.objects.all()) == 0:
        print("Reyslar haqidagi ma'lumotlarni bazaga yuklashni hohlaysizmi ? (y/n)")
...
# Foydalanuvchini kirish jarayoni
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            return render(request, "sky/login.html", {
                "message": "Xatolik username yoki parol xato."
            })
    else:
...
# Foydalanuvchini ro'yxatdan o'tish jarayoni
...
def register_view(request):
    if request.method == "POST":
        ...
        # Parolni tasdiqlash jarayoni
        password = request.POST["password"]
        confirmation = request.POST["confirmation"]
        if password != confirmation:
            return render(request, "sky/register.html", {
                "message": "Parollar bir xil bo'lishi kerak."
            })

        # Yangi foydalanuvchi yaratish jarayoni
        try:
            ...
            return render(request, "sky/register.html", {
                "message": "Bu username bilan kirilgan"
            })
        login(request, user)
        return HttpResponseRedirect(reverse("index"))
    else:
...
# Foydalanuvchi chiqish jarayoni
...
# Tanlangan yo'nalish bo'yicha reyslarni ko'rish jarayoni
...
def flight(request):
    ...
    try:
        ...
        return HttpResponse(f"Destination '{d_place.upper()}' not found.", status=404)

    try:
        origin = Place.objects.get(code=o_place.upper())
    except Place.DoesNotExist:
        return HttpResponse(f"Origin '{o_place.upper()}' not found.", status=404)
    ...
    if trip_type == '2':
        return render(request, "sky/search.html", {
            'flights': flights,
            'origin': origin,
            'destination': destination,
            'flights2': flights2,   ##
...
def review(request):
    ...
    if request.user.is_authenticated:
        ...
        #print("//////////////////////////////////")
        if round_trip:
            return render(request, "sky/book.html", {
                'flight1': flight1,
                'flight2': flight2,
...
def book(request):
    if request.method == 'POST':
        if request.user.is_authenticated:
            ...
            if f2:    ##
                return render(request, "sky/payment.html", { ##
                    'fare': fare+FEE,   ##
                    'ticket': ticket1.id,   ##
                    'ticket2': ticket2.id   ##
                })  ##
        ...
        return HttpResponse("Post manzil bo'lishi kerak.")
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                if t2:
                    ...
                    ticket2.save()
                    return render(request, 'sky/payment_process.html', {
                        'ticket1': ticket,
                        'ticket2': ticket2
                    })
                return render(request, 'sky/payment_process.html', {
...
def get_ticket(request):
    ...
    pdf = render_to_pdf('sky/ticket.html', data)
...
def bookings(request):
    if request.user.is_authenticated:
        ...
        return render(request, 'sky/bookings.html', {
            'page': 'bookings',
            'tickets': tickets
        })
    else:
...
# Chiptani bekor qilish jarayoni
...
def cancel_ticket(request):
    if request.method == 'POST':
        if request.user.is_authenticated:
            ...
            return HttpResponse("Foydalanuvchi ruxsatsiz")
    else:
        return HttpResponse("POST manzil bo'lishi kerak.")
    ...
    if request.method == 'POST':
        if request.user.is_authenticated:
            ...
            if ticket.user == request.user:
                ...
                return HttpResponse("Foydalanuvchi ruxsatisiz")
        else:
            return HttpResponseRedirect(reverse("login"))
    else:
        return HttpResponse("Post manzil bo'lishi kerak.")
...
def contact(request):
    return render(request, 'sky/contact.html')

def privacy_policy(request):
    return render(request, 'sky/privacy-policy.html')

def terms_and_conditions(request):
    return render(request, 'sky/terms.html')

def about_us(request):
    return render(request, 'sky/about.html')
